...
function xmlescape(str) { return str.replace(/[^ !#-;=?-~\n\r\t]/g, function(c) { return '&#' + c.charCodeAt(0) + ';'; }); }

// Parse a list of suites
function parseSuiteList(data) {
    // TODO: refactor this, such that it doesn't try to find tags
    var result = {};
    // remove comments
    data = data.replace(/#.*/g, '');
    // split on line
...
function runWithConfig(config) {
    ...
    var selenese = {};
    executeSuiteList(config);
    var suites;
    
    // read and execute a list of suites
    function executeSuiteList(config) {
        // TODO: probably move this code to outer function
    ...
    // TODO: This should probably be renamed to nextSuite()
    function testDone() {
        if(suites.length === 0) {
            ...
            executeSuite(config, suites.pop());
        }
    }
            ...
            Object.keys(results[suite]).forEach(function(testcase) {
                result.push('<testcase name="' + xmlescape(testcase) + '">');
                    results[suite][testcase].forEach(function(err) {
                        result.push('<failure>' + xmlescape(JSON.stringify(err)) + '</failure>');
                    });
