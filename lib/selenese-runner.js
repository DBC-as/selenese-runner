...
//
// TODO: introduction here with a bit about the structure and coding style of the program
//
...
// ## `runWithConfig` - the main function
//
...
// - `selenese` is the collection of selenese scripts, key is the filepath and value is a list of selenese commands
...
exports.runWithConfig = function(config) {
    ...
    // consume and execute the next suite in the `suites`-list. As the execution is asynchrous, this function will at the end of the callback.
    //
    // TODO: pass nextSuite throught the call-chain, instead of calling it directly to make flow more explicit.
    function nextSuite() {
        if(suites.length === 0) {
        ...
        request(baseurl + suite, function(err, res, data) {
            ...
            if(!data.match('<table id="suiteTable" cellpadding="1" cellspacing="1" border="1" class="selenium">')) {
                config.callback({testcase: 'error-before-testcase-reached'});
                config.callback({
                    error: "Testsuite doesn't look like a testsuite-file from selenium-IDE. Check that the url actually exists", 
                    url: baseurl + suite 
                });
                return nextSuite();
    ...
    function parseTests() {
        /* TODO: this should be moved to the end of the function  for more clear flow*/
        ...
        // read the file
        request(suiteurl + test, function(err, res, data) {
            if(!data || err) { 
                console.log(err, response, data); throw err; 
            }
    ...
    //
    // TODO: rename to prepareAndExecuteTests for more obvious flow.
    function prepareTests() {
        ...
        Object.keys(selenese).forEach(function(key) {
                selenese[key] = 
                    beforeEach.concat(selenese[key], afterEach);
        });
...
// ## `junitReporter` creates a junit-compatible report from the tests
// 
