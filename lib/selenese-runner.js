...
exports.runWithConfig = function(config) {
    ...
    function nextSuite(suite, callback) {
    ...
    function parseTests(testqueue, testcases, nextSuiteCallback) {
        ...
        if(testqueue.length === 0) {
            return prepareAndExecuteTests(testcases, 
                                          nextSuiteCallback);
        }
        ...
        read(suiteurl + test, function(err, data) {
            ...
            parseTests(testqueue, testcases, nextSuiteCallback);
        });
    }
    ...
    function executeNextTest(tests, nextSuiteCallback) {
        ...
        executeCommand(test.selenese.reverse(), function() {
                    executeNextTest(tests, nextSuiteCallback); 
        }, nextSuiteCallback);
    }
