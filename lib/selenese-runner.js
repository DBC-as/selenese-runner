...
exports.runWithConfig = function(config) {
    ...
    function nextSuite(suite, nextSuiteCallback) {
        ...
        read(baseurl + suite, function(err, data) {
            ...
            var testcases = {};
            consumeArray(testqueue, {
                each: function(elem, arrayCallback) {
                    parseTests(elem, testcases, 
                              arrayCallback, nextSuiteCallback);
                },
                done: function() {
                    prepareAndExecuteTests(testcases, 
                        nextSuiteCallback);
                }
            });
        }); 
    ...
    function parseTests(test, testcases, 
                        callback, nextSuiteCallback) {
    ...
    function prepareAndExecuteTests(testcases, nextSuiteCallback) {
        ...
        consumeArray(tests, {
            each: function(elem, callback) {
                executeNextTest(elem, callback, nextSuiteCallback);},
            done: nextSuiteCallback
        });
    }

    // run through the list of tests, 
    // and execute the selenese sequences
    function executeNextTest(test, callback, nextSuiteCallback) {
        ...
        executeCommand(test.selenese.reverse(), callback,  
                       nextSuiteCallback);
    }

    // Execute the next selenium command in a list of commands,
    // skipping to the next testcase og suite, if the command fails.
    function executeCommand(commands, nextTestCallback, 
                            nextSuiteCallback) {
        // Skip to the next testcase,
        ...
        if(command.command === 'restartBrowser') {
            browser.testComplete(function() {
                browser.session(function(err) {
                    ...
                    executeCommand(commands, nextTestCallback, 
                                   nextSuiteCallback);
                });
            });
            return;
...
function consumeArray(arr, args) {
    if(arr.length > 0) {
        args.each(arr.pop(), function() { 
                consumeArray(arr, args);
        });
    } else {
        args.done();
    }
